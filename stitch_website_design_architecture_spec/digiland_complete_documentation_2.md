...
> **Scope note:** *OCR & layout extraction* and *Field classification* are being built by another team member as a separate module. Our side does not implement these two steps — we **receive their structured JSON output** (per the uploaded file) as the starting input to the validation engine. They are shown below only to preserve the full pipeline context; treat them as an **external upstream input**, not part of our build.

```
Document upload (citizen/clerk)
        │
        ▼
[EXTERNAL MODULE — built by teammate]
...
Field classification      — NLP mapping to land-record fields
        │
        ▼
[OUR PIPELINE STARTS HERE — input = teammate's structured JSON output]
        ▼
...
2. **CV preprocessing** *(external module — teammate's build)* — deskewing, adaptive binarization, denoising, table/gridline detection, ROI segmentation (header / ledger table / margin notes / stamps).
3. **OCR extraction** *(external module — teammate's build)* — multilingual printed + handwritten recognition; every token tagged with its bounding box for later backtracking.
4. **NLP field classification** *(external module — teammate's build)* — raw tokens mapped into structured fields (khasra no., khata no., owner names, plot area, village/tehsil/district, land classification, mutation details).

   > **Our pipeline picks up from here.** Steps 2–4 above are delivered to us as a structured JSON payload (the teammate's output). We consume that JSON directly as the input to Step 5.
...
| Layer | Responsibility | Key technologies | Owner |
|---|---|---|---|
| Computer Vision | Deskew, binarize, denoise, table/layout segmentation, stamp/signature detection, map vectorization | OpenCV, Pillow, YOLOv8/Detectron2 | **External — teammate's module** |
| OCR | Printed + handwritten multilingual text recognition | EasyOCR, TrOCR | **External — teammate's module** |
| NLP / classification | Map recognized tokens to structured schema fields | spaCy, IndicNLP, LayoutLM, Hugging Face Transformers | **External — teammate's module** |
| Validation engine | Business rules, cross-database checks, duplicate detection | Python rules engine (Pydantic), PostGIS spatial queries | **Our pipeline (starts here)** |
| Confidence scoring | Composite scoring + routing | Custom weighted-score service | Our pipeline |
| Storage | Structured data + spatial data + raw files | PostgreSQL + PostGIS, Object storage (MinIO/S3) | Our pipeline |
| Backend / API | Orchestration, auth, integration | FastAPI | Our pipeline |
| GIS | Map rendering, spatial validation | GeoServer, Leaflet/OpenLayers | Our pipeline |
| Frontend | UI/UX | React.js | Our pipeline |
| Security | Encryption, signatures, audit | AES-256-GCM, ECDSA/Ed25519, SHA-256, mTLS, HMAC | Our pipeline |

### 5.2 Where Computer Vision specifically applies
*(Reference only — this entire section is delivered by a teammate's module, not built by us. Kept here so the full pipeline context is documented in one place.)*
...
### 5.3 Handoff contract — input to our pipeline

Since CV/OCR/NLP is a separate teammate's module, our pipeline's actual entry point is the **structured JSON output** that module produces. Recommended shape to agree on with them (adjust field names once their real output is confirmed):

```json
{
  "doc_id": "uuid",
  "source_document_sha256": "e3b0c44298fc1c...",
  "extracted_fields": {
    "khasra_no": "42/1",
    "khata_no": "104",
  ...
  "bounding_boxes": {
    "khasra_no": [xmin, ymin, xmax, ymax]
  }
}
```

`extracted_fields` feeds the validation engine, `field_confidence` feeds the OCR-confidence component of the composite score (§6.7), and `bounding_boxes` powers the split-view verification screen's zoom-crop (§3.4). Confirm the exact schema with the teammate building that module before wiring `POST /api/v1/process-document`.

### 5.4 Database design (consolidated)
...
| `POST /api/v1/process-document` | Receives the teammate's CV/OCR/NLP JSON output → runs validation → confidence scoring → saves structured record, all in one pipeline call |
